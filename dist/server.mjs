        ...
        io.on("connection", (socket) => {
            ...
            socket.on("message", async (msg) => {
                try {
                    ...
                    if (msg.senderUserId) {
                        io.to(`user-${msg.senderUserId}`).emit("newMessage", formattedMessage);
                    }
                    if (msg.senderAdminId) {
                        io.to(`admin-${msg.senderAdminId}`).emit("newMessage", formattedMessage);
                    }
                    if (msg.receiverUserId) {
                        io.to(`user-${msg.receiverUserId}`).emit("newMessage", formattedMessage);
                    }
                    if (msg.receiverAdminId) {
                        io.to(`admin-${msg.receiverAdminId}`).emit("newMessage", formattedMessage);
                    }
                    ...
                    if (msg.receiverUserId) {
                        ...
                        io.to(`user-${msg.receiverUserId}`).emit("messageListUpdate", updateForUser);
                        // If this is a new conversation, emit newConversation event
                        if (existingMessagesCount === 1) {
                            io.to(`user-${msg.receiverUserId}`).emit("newConversation", updateForUser);
                        }
                    }
                    if (msg.receiverAdminId) {
                        ...
                        io.to(`admin-${msg.receiverAdminId}`).emit("messageListUpdate", updateForAdmin);
                        // If this is a new conversation, emit newConversation event
                        if (existingMessagesCount === 1) {
                            io.to(`admin-${msg.receiverAdminId}`).emit("newConversation", updateForAdmin);
                        }
                    }
                    ...
                    if (msg.senderUserId) {
                        ...
                        const adminRooms = Array.from(io.sockets.adapter.rooms.keys()).filter((room) => room.startsWith("admin-"));
                    ...
                    if (msg.senderAdminId) {
                        ...
                        if (msg.receiverUserId) {
                            ...
                            console.log("Sending message update to user:", msg.receiverUserId, updateForUser);
                            io.to(`user-${msg.receiverUserId}`).emit("messageListUpdate", updateForUser);
                ...
                try {
                    ...
                    if (userId) {
                        ...
                        if (senderAdmin?.senderAdminId) {
                            io.to(`admin-${senderAdmin.senderAdminId}`).emit("messagesMarkedRead", {
                                projectId: projectId,
                                readByUserId: userId,
                            });
                        }
                        ...
                        if (senderUser?.senderUserId) {
                            io.to(`user-${senderUser.senderUserId}`).emit("messagesMarkedRead", {
                                projectId: projectId,
